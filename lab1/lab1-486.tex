...
The values for $M_p$ and $t_r$ are generally close to the theoretical values,
while $t_s$ displayed in many cases a high degree of difference, with a maximum error of
64\% for $\zeta = 0.7$ and in all but three cases an error above 15\%. 
There are some patterns to the variance across the three variables: 
$M_p$ tends to be consistently somewhat below the theoretical value, 
$t_r$ tends to be slightly higher, and 
$t_s$ tends to be longer. 
These variances are especially noticable at the higher $\zeta$ values and come closer to the  
theoretical values as $\zeta$ decreases. 
The increased settling time would tend to suggest there is some oscillation from within 
the circuitry of the analog computer counteracting the damping; however the increased rise
time and reduced peak would suggest increased damping. 
