...
% -------------------------------------------------------------%
function [Mp, tr, ts,MpIndex, t_10index,t_90index,tssIndex] = StepResponseMetrics(y,t, yStartIndex, ssVal, zeta)
...
t_90index  = find( y > 0.9*ssVal, 1, "first");
...
ss = 1:1:numel(t); % the final value
ss(:) = ssVal;
per105=1.05*ss;
per95=.95*ss;
per10=.10*ss;
...
% document tss
text(t(tssIndex),y(tssIndex), 'tss')
line([t(tssIndex);t(tssIndex)],[0,y(MpIndex)],...
        'Color','k','LineWidth',0.5,'LineStyle',':', 'HandleVisibility', 'off')

title({sprintf('M_p, t_r, and t_s for %s = %.1f', '\zeta', zeta)})
% Label the axes:
ylabel('t, time')  
xlabel('response to step in reference')

% make the plot line thicker
hold on
plot(t,y,'-','LineWidth',2, 'HandleVisibility', 'off')
hold off